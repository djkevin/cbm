...
class FormAPart1Controller {

    static allowedMethods = [save: "POST", update: "PUT", delete: "DELETE"]

    def index(Integer max) {
        params.max = Math.min(max ?: 10, 100)
        respond FormAPart1.list(params), model:[formAPart1InstanceCount: FormAPart1.count()]
    }

    def show(FormAPart1 formAPart1Instance) {
        respond formAPart1Instance
    }

    def create() {
        respond new FormAPart1(params)
    }

    @Transactional
    def save(FormAPart1 formAPart1Instance) {
    ...
    def edit(FormAPart1 formAPart1Instance) {
        respond formAPart1Instance
    }

    @Transactional
    def update(FormAPart1 formAPart1Instance) {
        if (formAPart1Instance == null) {
            notFound()
            return
        }
    ...
    @Transactional
    def delete(FormAPart1 formAPart1Instance) {
