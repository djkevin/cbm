...
<html>
	...
	<body>
		<a href="#list-formAPart1" class="skip" tabindex="-1"><g:message code="default.link.skip.label" default="Skip to content&hellip;"/></a>
...
<%--						<g:sortableColumn property="scope" title="${message(code: 'formAPart1.scope.label', default: 'Scope')}" />--%>
				...
				<tbody>
				<g:each in="${formAPart1InstanceList}" status="i" var="formAPart1Instance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
					
						<td><g:link action="show" id="${formAPart1Instance.id}">${fieldValue(bean: formAPart1Instance, field: "facilityName")}</g:link></td>
					
						<td>${fieldValue(bean: formAPart1Instance, field: "responsibleOrganisation")}</td>
					
						<td>${fieldValue(bean: formAPart1Instance, field: "location")}</td>
...
<%--						<td>${fieldValue(bean: formAPart1Instance, field: "scope")}</td>--%>
			...
			<div class="pagination">
				<g:paginate total="${formAPart1InstanceCount ?: 0}" />
			</div>
		</div>
	</body>
</html>
