...
%{--<div class="fieldcontain ${hasErrors(bean: reportInstance, field: 'stateParty', 'error')} required">--}%
	%{--<label for="stateParty" class="property-label25">--}%
		...
		%{--<span class="required-indicator">*</span>--}%
	%{--</label>--}%
	%{--<!--g:select id="stateParty1" name="stateParty.id" from="${cbm.StateParty.list()}" optionKey="id" required="" value="${reportInstance?.stateParty?.id}" class="many-to-one" readonly="readonly" /-->--}%
    %{--<!--g:field type="text" id="stateParty" name="stateParty.id" value="${reportInstance?.stateParty?.id}"  readonly="true" /-->--}%
